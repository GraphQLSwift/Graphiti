...
public struct StarWarsService : Service {
    public let root = API()
    public let context = Store()
    
    public init() {}
    
    public let schema = try! Schema<API, Store> { schema in
        ...
        schema.query { query in
            query.field(.hero, at: API.getHero)
            ...
            query.field(.human, at: API.getHuman)
                .argument(.id, at: \.id, description: "Id of the human.")

            query.field(.droid, at: API.getDroid)
                .argument(.id, at: \.id, description: "Id of the droid.")

            query.field(.search, at: API.search)
